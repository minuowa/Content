...
D3DXVECTOR3 CAreaCube::GeneratePos()
{
    ...
    vPos.x = gRandom.randF ( mfxMin, mfxMax );

    vPos.y = gRandom.randF ( mfyMin, mfyMax );

    vPos.z = gRandom.randF ( mfzMin, mfzMax );
...
D3DXVECTOR3 CAreaSphere::GeneratePos()
{
    ...
    int nRadius = gRandom.randI ( 0, LnRadius ) ;

    float fRad1 = gRandom.randF ( -D3DX_PI / 2.0f, D3DX_PI / 2.0f );
    float fRad2 = gRandom.randF ( 0, 2 * D3DX_PI );
...
D3DXVECTOR3 CAreaRect::GeneratePos()
{
    ...
    vPos.x = gRandom.randF ( 0, m_fWidth );
    vPos.y = gRandom.randF ( 0, m_fHeight );
    vPos.z = 0;
